...
static void init_display_controller(void)
{
    ...
    __delay_us(100);
    clcd_write(TWO_LINES_5x8_4_BIT_MODE, INST_MODE); // 0x20
    __delay_us(100);
...
void init_clcd (void)
{
    ...
    CLCD_DATA_PORT_DDR = CLCD_DATA_PORT_DDR & 0x0F; //TRISD TRISD

    //Setting the RS and EN lines as Output
    CLCD_RS_DDR = 0; // TRISE2
    CLCD_EN_DDR = 0; // TRISEL
...
void clcd_putch(const char data, unsigned char addr)
{
    ...
    clcd_write(data, DATA_MODE); // Aas date
}

//glcd_print("hello", LINE2(0));
void clcd_print(const char *str, unsigned char addr)
{
    clcd_write(addr, INST_MODE); // 0x00

    while (*str != '\0')
    {
        clcd_write(*str, DATA_MODE); // h, e, 1, 1, o, \o
        str++;
    }
}